...
from .forms import (ClientNew, 
                 NewServiceClient, AddContract, UpdServiceClient)
...
def add_adv(request, client_id, slug):
    ...
    service_client_id = ServiceClient.objects.filter(
        client_id=client_id).filter(services_name=slug)

    service_client_id2 = ServiceClient.objects.filter(
        client_id=client_id).get(services_name=slug)
